...
//		String[] list = {"냉장고", "로봇청소기", "세탁기", "에어컨"};
//		String[] list2 = {"노트북", "TV", "에어컨", "플레이스테이션", "로봇청소기"};
		...
		for (int i = 0; i < playList.size(); i++) { // arraylist니깐 size로
			System.out.println(playList.get(i));
		}

		System.out.println("\n==============정렬==============\n");

		Collections.sort(playList); // ABC > abc > 가나다 순으로 정렬
	...
	public static ArrayList<String> searchSong(String option, String keyword, ArrayList<String> playList) {
		...
		for(int i = 0; i < playList.size(); i++) {  //	playList.get(i); = "INVU : 태연"
	...
	public static ArrayList<String> searchSong(String keyword, ArrayList<String> playList) { // "사랑"이 포함된 노래

		ArrayList<String> result = new ArrayList<>();
		// playList 내에 keyword를 포함하고 있는 데이터를 찾기
		for (int i = 0; i < playList.size(); i++) {
			if (playList.get(i).contains(keyword)) {
				result.add(playList.get(i)); // result에 해당 데이터 담기
			}
		}
		return result;
	}
