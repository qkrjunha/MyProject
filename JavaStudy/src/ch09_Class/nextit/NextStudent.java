...
public class NextStudent {
	...
	// 생성자(Constructor) 만들기 - 단축키[Alt + Shift + S] 또는 마우스 우클릭 -> Source
	...
	public NextStudent(String name, int age) {
		this(name, age, 1, 0 );
//		this.name = name; 이거 두개 대신 위에 this(name, age, 1, 0 );로 해도된다.
//		this.age = age;
// level과 exp를 입력받지 않았다면 1과 0 담기 
//		this.level = 1; 위에 int level =1;로 선언했으면 안써도되고 안썻으면 쓰면된다.
//		this.exp = 0;
	...
	public void endDay() {
		...
		exp += rand;
		System.out.println(name + "씨의 현재 경험치는" + exp);
		if (exp >= 100) {
			System.out.println("레벨업!");
			level++;
			System.out.println(name + "씨의 현재 레벨은" + level);
			exp -= 100;
		}
