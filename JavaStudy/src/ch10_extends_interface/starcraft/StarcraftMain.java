...
//		protossList.add(hightemplar);
		...
		// HightTemplar (자식) 이 StarUnit (부모) 로 
		// 형변환이 되어있기 때문에 
		// 부모가 가지고 있지 않은 메소드를 사용불가
...
//		((HighTemplar)starList.get(4)).thunderStorm(); 도 가능
		
		// for문을 돌려서 이 객체가 원래는 하이템플러 였다면 썬더스톰을 쓰도록 해보자
		
		// 하이템플러 -> 스타유닛 -> 하이템플러 가능
		// 하이템플러 -> 스타유닛 -> 질럿, 마린 불가능
//		((HighTemplar)starList.get(0)).move(5, 5);
		...
		for(int i =0; i<starList.size(); i++) {
			...
			// 가능하다면 true, 불가능하면 false 리턴하는 형태
